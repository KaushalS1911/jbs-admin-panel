...
const AttendanceView = ({ option, startDate, setSelect }) => {
  const [batchview, setBatchviewData] = useState([]);
  const [checked, setChecked] = useState({});
  const [profileData] = useRecoilState(profile);
  const allStatus = ["Present", "Absent", "Late"];
  ...
  useEffect(() => {
    // Reset batchview and checked state when option or startDate changes
    const initialBatchview = option.batch_members
      ? option.batch_members.map((item) => ({
          studentId: item.student_id,
          status: "",
          date: startDate,
          company_id: profileData.company_id,
      ...
      : [];
    setBatchviewData(initialBatchview);

    const initialChecked = option.batch_members
      ? option.batch_members.reduce((acc, item, index) => {
          acc[index + 1] = "";
          return acc;
        }, {})
      : {};
    setChecked(initialChecked);
  }, [option, startDate, profileData.company_id]);

  const handleData = (e, params) => {
    const updatedStatus = e.target.value;
    const studentId = params.row.studentId;

    const updatedBatchview = batchview.map((item) =>
      item.studentId === studentId ? { ...item, status: updatedStatus } : item
    );
    setBatchviewData(updatedBatchview);

    setChecked((prev) => ({
      ...prev,
      [params.row.id]: updatedStatus,
    }));
  };
  ...
  const handleActionSubmit = () => {
    const apiEndpoint = `${process.env.REACT_APP_API_URL}attendance`;
    axios
      .post(apiEndpoint, { attendance: batchview })
      .then((res) => {
        openNotificationWithIcon("success", res.data.data.message);
        setSelect("");
      })
      .catch((error) => {
        openNotificationWithIcon("error", "Failed to submit attendance");
      });
