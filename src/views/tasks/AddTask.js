...
const AddTask = ({ openAddTaskDialog, setOpenAddTaskDialog, refetch }) => {
  ...
  const handleRoleChange = (event) => {
    instance
      .get(
        `${process.env.REACT_APP_LOGIN_URL}users/company/${user.company_id}/role/${event.target.value}`
      )
      .then((response) => {
  ...
  const handleUserChange = (event) => {
    const selectedUser = formik.values.options.find(
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await axios.post(
          `${process.env.REACT_APP_API_URL}${user.company_id}/task`,
          payload
        );
        formik.resetForm();
        handleClose();
              ...
              <Grid item xs={6} style={{ padding: "5px", marginBottom: "5px" }}>
                <FormControl fullWidth>
                  ...
                  {formik.touched.role && formik.errors.role && (
                    <small style={{ color: "red", margin: "5px 0px 0px 10px" }}>
                      {formik.errors.role}
                    </small>
                  )}
                </FormControl>
