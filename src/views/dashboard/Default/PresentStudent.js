  ...
  useEffect(() => {
    if (data && Array.isArray(data)) {
      const filterStudents = data.filter(
        (entry, index, arr) => arr.findIndex((e) => e.studentId === entry.studentId) === index
      );
        ...
        <Grid container direction="row" justifyContent="space-around" alignItems="center">
          ...
          <Grid item>
            <Grid container direction="column">
              <Typography sx={{ fontSize: "14px", fontWeight: 500, color: "#1B1D28" }}>
                P/A Student
              </Typography>
              <Typography
                sx={{ fontSize: "26px", fontWeight: 500, textAlign: "center", color: "#1B1D28" }}
              >
