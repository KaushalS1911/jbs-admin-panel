...
function EmpRoles() {
    ...
    function handleDelete(item){
        const filteredEmpRoles = configs.emp_type.filter((e) => e !== item)
        const apiEndpoint = `${process.env.REACT_APP_API_URL}${configs.company_id}/configs/${configs._id}`;
        const payload = {...configs, emp_type: filteredEmpRoles}
        axios.put(apiEndpoint, payload).then((res) => {
            dispatch(getConfigs())
        }).catch((err) => console.log(err))
    }
    ...
    return (
        <>
            <PageTitle title={"Settings"} subtitle={"Employee Roles"}/>
            ...
            <MainCard sx={{width: "100%", maxWidth: "600px", padding: "10px"}}>
                ...
                <List >
                    {
                        configs?.emp_type && configs?.emp_type.length !== 0 && configs?.emp_type.map((e) => {
                            return(
