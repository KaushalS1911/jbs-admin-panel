...
function CalendarApp() {
  ...
  async function handleAddEvent(values) {
    ...
    try {
      await axios.post(`${process.env.REACT_APP_API_URL}${user.company_id}/event`, payload);
      refetch();
      handleClose();
    } catch (error) {
  ...
  const handleDeleteEvent = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}${user.company_id}/${selectedEvent}/deleteEvent`);
      refetch();
