import {
  Avatar, Button, Checkbox, Dialog, DialogActions,
  DialogContent,
  DialogContentText,
  DialogTitle,
  FormControl,
...
const AddSeminar = ({ openAddTaskDialog, setOpenAddTaskDialog, refetch, setIsEdit, isEdit, editId, setEditId }) => {
  ...
  const handleRoleChange = (event) => {
    ...
    if (selectedRole) {
      instance.get(`${process.env.REACT_APP_LOGIN_URL}users/company/${user.company_id}/role/${selectedRole}`)
        .then(response => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        if (isEdit) {
          await axios.put(`${process.env.REACT_APP_API_URL}${user.company_id}/${editId}/updateseminar`, payload);
        } else {
          await axios.post(`${process.env.REACT_APP_API_URL}${user.company_id}/seminar`, payload);
        }
        if (!isEdit) {
          formik.resetForm()
        }
  ...
  const handleUserChange = (event) => {
    ...
    formik.setFieldValue('user', typeof value === 'string' ? value.split(',') : value);
  };

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get(`${process.env.REACT_APP_API_URL}${user.company_id}/${editId}/${user._id}/seminar`);
        const semi = response.data.data.seminars;
        const myRole = await instance.get(`${process.env.REACT_APP_LOGIN_URL}users/company/${user.company_id}/role/${semi[0]?.attended_role}`)
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${process.env.REACT_APP_API_URL}${user.company_id}/${editId}/deleteseminar`);
  ...
  return (
    <>
          ...
          <form onSubmit={formik.handleSubmit}>
            ...
            >
              <Grid container sx={{ display: 'flex' }}>
                  ...
                  <Grid container sx={{ padding: "0px 15px " }}>
                    ...
                    >
                      <TextField
                        ...
                        onChange={formik.handleChange}
                        onBlur={formik.handleBlur}
                        error={formik.touched.schedule_by && Boolean(formik.errors.schedule_by)}
                        helperText={formik.touched.schedule_by && formik.errors.schedule_by}
                        InputLabelProps={{
                          style: { color: "#5559CE" },
                        }}
                      />
                    ...
                    >
                      ...
                      >
                        <Flatpickr
                          ...
                          onChange={(date) => formik.setFieldValue('date_time', date[0])}
                          options={{
                            enableTime: true,
                        ...
                        {formik.touched.date_time && formik.errors.date_time && (
                          <small style={{ color: 'red', margin: '5px 0px 0px 10px' }}>{formik.errors.date_time}</small>
                        )}
                      </FormControl>
                    </Grid>
                  </Grid>
                      ...
                      <FormControl fullWidth>
                        ...
                        <Select
                          ...
                          renderValue={(selected) => {
                            const selectedUsers = formik.values.options.filter((user) => selected.includes(user._id));
                ...
                <Grid xs={6} container sx={{ padding: "15px 15px 0px 15px" }}>
                  <>
                    {
                      formik.values.user.length > 0 ? (
                        <>
                          <Grid item xs={12} sx={{ display: 'flex', alignItems: 'center', position: 'fixed', backgroundColor: '#fff' }}>
                            <Avatar sx={{ backgroundColor: '#D9DAF9', width: 26, height: 26, color: 'black' }}></Avatar> <Typography variant="h5" sx={{ marginLeft: '10px' }}>Selected member</Typography>
                          </Grid>
                          <Grid xs={12} sx={{ overflow: 'scroll', height: 280, marginTop: 5 }}>
                            <TableContainer component={Paper}>
                              <Table>
                                <TableBody>
                                  {Array.isArray(formik.values.user) && formik.values.user.map((userId) => {
                                    const selectedUser = formik.values.options.find(user => user._id === userId);
                                    return (
                                      <TableRow key={userId}>
                                        <TableCell style={{ padding: '10px' }}> {selectedUser ? `${selectedUser.firstName} ${selectedUser.lastName}` : 'User not found'}</TableCell>
                                      </TableRow>
                                    );
                                  })}
                                </TableBody>
                        ...
                        <>
                          <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center' }}>
                            <img src={emprty} alt='no data' loading="lazy" style={{ maxWidth: '250px' }} />
                          </div>
                        </>
                      )
                    }
      ...
      >
        ...
        <DialogActions>
          <Button onClick={handleCloseDeleteConfirmation} style={{ color: "#5559CE" }}>
            Cancel
          </Button>
